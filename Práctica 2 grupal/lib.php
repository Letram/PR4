...
<body>";
    ...
    public static function createUserForm(){
        echo "<form action=\"\" method=\"post\">
                <div class=\"log a\">
                    <strong>Usuario: *</strong> <input class=\"inputText\" type=\"text\" name=\"user\"/ required><br/>
                    
                    <strong>Clave: *</strong> <input class=\"inputText\" type=\"password\" name=\"pass\"/ required><br/>
                    
                    <strong>Nombre: *</strong> <input class=\"inputText\" type=\"text\" name=\"name\"/ required><br/>
                    
                    <strong>Tipo: *</strong> <input class=\"inputText\" type=\"number\" name=\"type\" min=\"1\" max=\"3\" value=\"1\" / required><br/>
    ...
    public static function showDrinkTable($opt=1){
        ...
        if($opt == 0){
            echo "<tr colspan='6'><td><form method='post' action=''>
                        Población: <input type='text' name='poblation' required><br>
                        Dirección: <input type='text' name='address' required><br>
                        <input class=\"button\" type='submit' name='endOrder' value='Finalizar pedido'>
                  </form></td></tr>";
        }
    }
    public static function showOrderTable($id){
        ...
        $res=DB::execute_sql("select id, horacreacion as [hora de creacion], poblacionentrega as [poblacion], direccionentrega as [direccion], horaasignacion as [hora de asignacion], horareparto as [hora de reparto], horaentrega as [hora de entrega], pvp from pedidos where idcliente=?", array($id));
        if($res){
            $res->setFetchMode(PDO::FETCH_NAMED);
            $first=true;
            echo "<table><tr>";
            foreach ($res as $table_row) {
                ...
                if($table_row['hora de creacion'] != 0){
                    ...
                    echo "<td><a href='placeOrder.php?id=".$table_row['idcliente']."&orderid=".$table_row['id']."'>Sin finalizar</a></td></tr>";
                }
            }
            echo "</table>";
        }
